...
interface CreditCardStatementImportModalProps {
  isOpen: boolean;
  onClose: () => void;
  creditCards: CreditCard[];
}

export default function CreditCardStatementImportModal({ isOpen, onClose, creditCards }: CreditCardStatementImportModalProps) {
  const { addCreditCardTransaction } = useFirebase();
  ...
  const [parsedTransactions, setParsedTransactions] = useState<Partial<CreditCardTransaction>[]>([]);
  ...
  const handleParse = async (force = false) => {
    ...
    try {
      ...
      const response = await callAIWithRetry(() => ai.models.generateContent({
        ...
        contents: `Extract credit card transactions from the following text. 
        Return a JSON array of objects with fields: date (YYYY-MM-DD), description, amount (number), type ('Debit' for spending, 'Credit' for payments/refunds), category (e.g., Food, Travel, Shopping, Utilities, Entertainment, Health, etc.).
      ...
      >
        ...
        <div className="p-6 border-b border-gray-100 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-xl font-bold text-gray-900">Import Card Statement</h3>
              <p className="text-sm text-gray-500">AI-powered credit card statement parsing</p>
            </div>
          </div>
          ...
          {step === 'input' && (
            <div className="space-y-6">
              <div className="space-y-2">
                <label className="text-sm font-medium text-gray-700">Select Credit Card</label>
                <select
                ...
                >
                  {creditCards.map(card => (
                    <option key={card.id} value={card.id}>{card.cardName} - {card.bankName} (**** {card.lastFourDigits})</option>
                  ))}
                </select>
              </div>
                ...
                {importMethod === 'text' ? (
                  <div className="space-y-2">
                    ...
                    <textarea
                      ...
                      placeholder="Paste text from your credit card statement, email, or SMS here..."
          ...
          {step === 'success' && (
            <div className="py-12 text-center space-y-4">
              ...
              <p className="text-gray-500 max-w-xs mx-auto">
                {parsedTransactions.length} transactions have been successfully added to your card.
              </p>
            </div>
          )}
        </div>
